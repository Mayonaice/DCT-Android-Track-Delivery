...
class _DeliveryStatusDetailPageState extends State<DeliveryStatusDetailPage> {
  ...
  Map<String, String> _photo64ByFilename = {};
  ...
  Map<String, String> _buildPhoto64ByFilename(List<DeliveryStatusDetailData> statuses) {
  ...
  StatusPhoto? _firstStatusPhotoWhereOrNull(
    List<StatusPhoto> photos,
    bool Function(StatusPhoto) test,
  ) {
  ...
  List<StatusPhoto> _selectPhotosForStatus(DeliveryStatusDetailData status) {
    ...
    for (final photo in candidates) {
      ...
      if (photo64.isEmpty) {
        final key = _buildPhotoKey(status.deliveryCode, photo.filename);
        photo64 = _photo64ByFilename[key] ?? '';
      }
      if (photo64.isEmpty) continue;
    ...
    final fallback = _firstStatusPhotoWhereOrNull(status.photo, (p) => p.photo64.isNotEmpty) ??
    ...
    if (fallbackPhoto64.isEmpty) {
      ...
      fallbackPhoto64 = _photo64ByFilename[key] ?? '';
    }
  ...
  Future<void> _loadStatusDetail() async {
    ...
    try {
      ...
      if (response != null && response.ok && response.data.isNotEmpty) {
        final reversed = response.data.reversed.toList();
        setState(() {
          _statusData = reversed; // Reverse untuk urutan dari bawah ke atas
          _photo64ByFilename = _buildPhoto64ByFilename(reversed);
  ...
  Widget _buildTimelineItem(DeliveryStatusDetailData data, {bool isLast = false}) {
    final selectedPhotos = _selectPhotosForStatus(data);
                ...
                children: [
                  ...
                  if (selectedPhotos.isNotEmpty)
                    Container(
                      ...
                      child: InkWell(
                        onTap: () => _showPhotoModal(selectedPhotos),
                        borderRadius: BorderRadius.circular(6),
                        child: const Padding(
